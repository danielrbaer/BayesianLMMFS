...
#'
#' #Inspect estimated leave-one-out cross-validation measure re: the longitudinal outcome data.
#' #This uses the \code{loo} R package developed by Vehtari et al.
#'
#' model_results$MCMC_summary%>%
#' dplyr::select(elpd_loo)%>%
#' unique%>%
#' dplyr::mutate(looic=-2*elpd_loo)
#'
#' @references
#' \itemize{
#'
...
BayesianLMMFS<-function(
  ...
  #Source C++ functions re: MCMC updates:####

  paste('Source Rcpp code')

  #MVN sampler:

  Rcpp::sourceCpp("simMVN.cpp")

  #------#

  #IW sampler:

  Rcpp::sourceCpp("simIW.cpp")
  ...
  X_kron_mat<-
    ss_data_matrix_fn(data_matrix=X_mat,
                      J=J)
  ...
  X_k_SS<-GROUP_data_matrix_fn(data_matrix=X_mat,
                               J=J,
                               X_dim_df = X_dim_df)
  ...
  X_l_SS<-
    BI_data_matrix_fn(data_matrix=X_mat,
                      J=J,
                      inference=inference)
  ...
  Z_mat<-
    Z_data_matrix_fn(data_list=Z_list,
                     J=J,
                     q=q)
  ...
  if(raw_MCMC_output==T){
  ...
  }else if(just_Beta_raw_MCMC_output==T){
    ...
    return(
      ...
      list(
        ...
        'MCMC_output'=MCMC_output_df%>%
          dplyr::filter(parameter=='beta',
                        feature%in%selected_features_mcmc),

        'MCMC_summary'=MCMC_summary_df_TOTAL
